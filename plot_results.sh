...
<body class="bg-gray-900">
    ...
    <script type="text/babel" data-presets="react">
        // The entire React application is embedded here.
        // It uses the data read from the input file.

        const { useState, useMemo } = React;
        const { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, Cell } = Recharts;

        // The benchmark data is injected here from the shell script
        ...
        function App() {
          ...
          const benchmarks = useMemo(() => [...new Set(allData.map(item => item.benchmark).filter(Boolean))], [allData]);
          const [selectedBenchmark, setSelectedBenchmark] = useState(benchmarks.length > 0 ? benchmarks[0] : '');

          const chartData = useMemo(() => 
            allData
              .filter(d => d.benchmark === selectedBenchmark)
              .sort((a, b) => b.ops - a.ops),
            [allData, selectedBenchmark]
          );
                      ...
                      <BarChart data={chartData} margin={{ top: 5, right: 20, left: 30, bottom: 5 }} barCategoryGap="20%">
                        ...
                        <Bar dataKey="ops" name="Ops/sec">
                           {chartData.map((entry, index) => (
                            <Cell key={\`cell-\${index}\`} fill={COLORS[entry.storage] || '#8884d8'} />
                          ))}
                        </Bar>
                        <Legend content={() => {
